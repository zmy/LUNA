...
from .utils import BertForPreTraining, BertForMaskedLM, TRANSFORMER_VERSION, TransformerVersion
from .table_bert import TableBertModel
from .config import TableBertConfig, BERT_CONFIGS
...
class VanillaTableBert(TableBertModel):
    ...
    def load_state_dict(self, state_dict: Dict[str, Any], strict: bool = True):
        if not any(key.startswith('_bert_model') for key in state_dict):
            ...
            print(super(VanillaTableBert, self).load_state_dict(state_dict, strict=False))
            self._bert_model.resize_token_embeddings(len(self.tokenizer))
