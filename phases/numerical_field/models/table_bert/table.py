...
class Table(object):
    ...
    def tokenize(self, tokenizer: BertTokenizer):
        for column in self.header:
            new_name,number_list=parse_string(column.name,128,3)
            column.name=new_name
            column.name_tokens = tokenizer.tokenize(column.name)
            column.number_list=number_list
        tokenized_rows = []
        number_rows=[]
        for row in self.data:
            ...
            number_row=[]
            for v in row:
                new_name, number_list = parse_string(v,128,3)
                tokenized_row.append(tokenizer.tokenize(new_name))
                number_row.append(number_list)
            tokenized_rows.append(tokenized_row)
            number_rows.append(number_row)

        self.data = tokenized_rows
        self.data_number=number_rows
