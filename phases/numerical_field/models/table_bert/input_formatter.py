...
class VanillaTableBertInputFormatter(TableBertBertInputFormatter):
    def get_cell_input(
        ...
        return input, span_map,number_list
    ...
    def get_row_input(self, context,context_number, header: List[Column], row_data: List[Any], row_number: List[Any], trim_long_table=False,max_length=343):
        if self.config.context_first:
            ...
            max_table_token_length = max_length - len(context) - 2 - 1
        else:
        ...
        for col_id, column in enumerate(header):
            column_input_tokens, token_span_map ,number_list= self.get_cell_input(
                column,
                row_data[col_id],
                row_number[col_id],
            ...
            row_input_numbers.extend(number_list)
