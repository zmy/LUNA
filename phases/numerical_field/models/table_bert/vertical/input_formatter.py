...
from ..input_formatter import TableBertBertInputFormatter, VanillaTableBertInputFormatter
...
class VerticalAttentionTableBertInputFormatter(VanillaTableBertInputFormatter):
    ...
    def get_input(self, context, table: Table,max_length=343):
        row_instances = []

        table_data = table.data[:20]
        number_data=table.data_number[:20]
        for row_data,row_number in zip(table_data,number_data):
            row_instance = self.get_row_input(context, table.header, row_data,row_number,max_length=max_length)
    ...
    def get_row_input(self, context, header: List[Column], row_data: List[Any],row_number: List[Any],max_length=343):
        row_instance = super(VerticalAttentionTableBertInputFormatter, self).get_row_input(
            context[0],context[1], header, row_data=row_data,row_number=row_number,max_length=max_length)
