...
from ..vanilla_table_bert import VanillaTableBert, VanillaTableBertInputFormatter, TableBertConfig
from ..table import *
from ..vertical.config import VerticalAttentionTableBertConfig
from ..vertical.input_formatter import VerticalAttentionTableBertInputFormatter
from ..vertical.dataset import collate
...
class VerticalAttentionTableBert(VanillaTableBert):
    ...
    def forward_embeds(self,inputs_embeds,segment_ids,sequence_mask,max_row_num,column_token_position_to_column_ids,table_mask,context_token_positions,context_token_mask):
        ...
        batch_size, sequence_len, hidden_size=bert_output.size(0)//max_row_num,bert_output.size(1),bert_output.size(2)
        bert_output = bert_output.view(batch_size, max_row_num, sequence_len,hidden_size)
    ...
    def to_tensor_dict(
        ...
        for e_id, (context, table) in enumerate(zip(contexts, tables)):
            instance = self.input_formatter.get_input(context, table,max_length=max_length)
        ...
        tensor_dict = collate(examples, train=False)
    ...
    def encode(
        ...
        for k,v in tensor_dict.items():
            if k!='number_list':
                print(k,v.size())
        print(tensor_dict['segment_ids'])
        print(tensor_dict['input_ids'])
        tensor_dict = {
