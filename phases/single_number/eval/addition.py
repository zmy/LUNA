...
from data.dataset.add_dataset import AddDataset, collate
from torch.utils.data import DataLoader
from tqdm import tqdm

from tabular_model.number_encoder import Addition
from tabular_model.number_encoder import Addition_Config
...
def eval(args):
    ...
    train_dataset = AddDataset(dataset_name, is_train=True, preprocess_type=preprocess_type)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=16, pin_memory=True, shuffle=True,
                              collate_fn=collate)
    valid_dataset = AddDataset(dataset_name, is_train=False, preprocess_type=preprocess_type)
    ...
    for epoch in range(epoch_num):
        ...
        # for (number, seq_len, sig_gt, exp_gt, val_gt, number_string) in tqdm(train_loader):
        for (numbers, results) in tqdm(train_loader):
            inputs = []
            for number in numbers:
                token_ids, seq_len, sig_gt, exp_gt, val_gt, number_string = number
                ...
                inputs.append((token_ids, seq_len, sig_gt, exp_gt, val_gt, input_ids, attention_mask))

            sig_pred, exp_pred = model(inputs)
            val_gt, sig_gt, exp_gt = results
            val_gt = val_gt.to(device)
            sig_gt = sig_gt.to(device)
            exp_gt = exp_gt.to(device)
