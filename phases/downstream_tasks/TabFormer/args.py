...
def define_main_parser(parser=None):
    ...
    parser.add_argument("--per_device_train_batch_size", type=int, default=20, help="batch size on a single device")
    parser.add_argument("--per_device_eval_batch_size", type=int, default=20, help="batch size on a single device")

    # Number Net arguments.
    parser.add_argument('--use_numtok', action='store_true', help='add number net into tabformer')
    parser.add_argument('--use_replace', action='store_true', help='add replace policy into tabformer')
    parser.add_argument('--number_model_config', type=str, default='CharLSTM_base_for_tabformer',
                        help='configuration for NumBed')
    parser.add_argument('--use_reg_loss', action='store_true', help='use regression loss in pre-training')
