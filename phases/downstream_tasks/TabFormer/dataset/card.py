...
class TransactionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_numtok:
            # Returns tokenized rows and raw numbers from the numerical columns.
            return_data = {
                'input_ids': return_data,
                'raw_num': self.nums[index]
            }

        if self.return_labels:
            # Returns sample (window) level label for Fraud Detection Task.
            return_data = (return_data, torch.tensor(self.window_label[index], dtype=torch.long))
    ...
    def save_vocab(self, vocab_dir):
        ...
        self.log.info(f"saving vocab at {file_name}")
    ...
    def user_level_data(self):
        ...
        trans_data, trans_labels, trans_nums = [], [], []

        if self.cached and path.isfile(fname):
            self.log.info(f"loading cached user level data from {fname}")
            ...
            for user in tqdm.tqdm(unique_users):
                ...
                for idx, row in user_data.iterrows():
                    ...
                    user_trans.extend(row[skip_idx:11])
                    if self.use_numtok:
                        user_nums.extend(row[12:])
                    user_labels.append(row[11])

                trans_data.append(user_trans)
                trans_labels.append(user_labels)
                if self.use_numtok:
                    trans_nums.append(user_nums)

            if self.skip_user:
                columns_names.remove("User")

            if self.use_numtok:
                columns_names.remove('Raw Time')
                columns_names.remove('Raw Amount')
                columns_names.remove('Raw Zip')
                columns_names.remove('Raw MCC')

            with open(fname, 'wb') as cache_file:
                if self.use_numtok:
                    pickle.dump(
                        {"trans": trans_data, "labels": trans_labels, "columns": columns_names, 'nums': trans_nums},
                        cache_file)
                else:
        ...
        # convert to str
        if self.use_numtok:
            return trans_data, trans_labels, columns_names, trans_nums
        else:
    ...
    def prepare_samples(self):
        self.log.info("preparing user level data...")
        if self.use_numtok:
            trans_data, trans_labels, columns_names, trans_nums = self.user_level_data()
        else:
            trans_data, trans_labels, columns_names = self.user_level_data()

        # TODO: Get a subset of data for quicker debugging (e.g. trans_data[:50])
        self.log.info("creating transaction samples with vocab")
        for user_idx in tqdm.tqdm(range(len(trans_data))):
            ...
            if self.use_numtok:
                user_nums = trans_nums[user_idx]
                user_nums = np.array(user_nums).reshape(-1, 4).tolist()
            ...
            if self.use_numtok:
                for jdx in range(0, len(user_labels) - self.seq_len + 1, self.trans_stride):
                    ids = user_nums[jdx:(jdx + self.seq_len)]
                    self.nums.append(ids)

                assert len(self.data) == len(self.nums)
        ...
        self.log.info(f"ncols: {self.ncols}")
        self.log.info(f"no of samples {len(self.data)}")

    def get_csv(self, fname):
        data = pd.read_csv(fname, nrows=self.nrows)
        if self.user_ids:
            self.log.info(f'Filtering data by user ids list: {self.user_ids}...')
            self.user_ids = map(int, self.user_ids)
            data = data[data['User'].isin(self.user_ids)]

        self.nrows = data.shape[0]
        self.log.info(f"read data : {data.shape}")
        return data

    def write_csv(self, data, fname):
        self.log.info(f"writing to file {fname}")
        data.to_csv(fname, index=False)

    def init_vocab(self):
        column_names = list(self.trans_table.columns)
        if self.use_numtok:
            # Raw numbers are excluded from the vocabulary.
            column_names.remove("Raw Time")
            column_names.remove("Raw Amount")
            column_names.remove("Raw Zip")
            column_names.remove("Raw MCC")
        ...
        self.log.info(f"total columns: {list(column_names)}")
        self.log.info(f"total vocabulary size: {len(self.vocab.id2token)}")

        for column in self.vocab.field_keys:
            vocab_size = len(self.vocab.token2id[column])
            self.log.info(f"column : {column}, vocab size : {vocab_size}")

            if vocab_size > self.vocab.adap_thres:
                self.log.info(f"\tsetting {column} for adaptive softmax")
    ...
    def encode_data(self):
        ...
        if self.cached and path.isfile(path.join(dirname, fname)):
            self.log.info(f"cached encoded data is read from {fname}")
        ...
        self.log.info(f"{data_file} is read.")

        self.log.info("nan resolution.")
        if self.use_numtok:
            data['Raw Amount'] = data['Amount']
            data['Raw Zip'] = self.nanZero(data['Zip'])
            data['Raw MCC'] = data['MCC']
        ...
        self.log.info("label-fit-transform.")
        ...
        self.log.info("amount quant transform")
        ...
        if self.use_numtok:
            # Keep the raw numbers of the numerical field 'Timestamp', 'Amount', 'Zip' and 'MCC'.
        ...
        self.log.info(f"writing cached csv to {path.join(dirname, fname)}")
        if not path.exists(dirname):
            os.mkdir(dirname)
        self.write_csv(self.trans_table, path.join(dirname, fname))

        encoder_fname = path.join(dirname, f'{self.fname}{self.fextension}.encoder_fit.pkl')
        self.log.info(f"writing cached encoder fit to {encoder_fname}")
